...
export default function Main({
  ...
  const updateStash = (warpItem) => {
    ...
    // Convert items
    const itemCQN = stash[warpItem];
    if (warpItem === "candy-bag" || (warpItem === "legendary-item" && itemCQN > 0)) {
      stash["stellar-candy"] += 40;
    }
    else if (warpItem.includes("normal-item") && itemCQN > 0) {
      stash["stellar-candy"] += 5;
    }
    else if (warpItem.includes("pumpkin-piece") && itemCQN > 0) {
      stash["stellar-candy"] += 3;
    }
    else {
      stash[warpItem]++;
    }
